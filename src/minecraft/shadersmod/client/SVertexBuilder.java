...
public class SVertexBuilder
{
  ...
  public static void endSetVertexFormat(WorldRenderer wrr)
  {
    ...
    vertexSize = (vf.func_177338_f() / 4);
    hasNormal = vf.func_177350_b();
    hasTangent = hasNormal;
    hasUV = vf.func_177347_a(0);
    offsetNormal = (hasNormal ? vf.func_177342_c() / 4 : 0);
    offsetUV = (hasUV ? vf.func_177344_b(0) / 4 : 0);
    offsetUVCenter = 8;
  }
  ...
  public static void endAddVertex(WorldRenderer wrr)
  {
    ...
    if (vertexSize == 14)
    {
      if ((drawMode == 7) && (vertexCount % 4 == 0))
      {
        svb.calcNormal(wrr, rawBufferIndex - 4 * vertexSize);
      }
      
      long eData = entityData[entityDataIndex];
      int pos = rawBufferIndex - 14 + 12;
      rawIntBuffer.put(pos, (int)eData);
      rawIntBuffer.put(pos + 1, (int)(eData >> 32));
    }
  }
  ...
  public static void endAddVertexData(WorldRenderer wrr)
  {
    ...
    if ((vertexSize == 14) && (drawMode == 7) && (vertexCount % 4 == 0))
    {
      svb.calcNormal(wrr, rawBufferIndex - 4 * vertexSize);
    }
  }
  ...
  {
    FloatBuffer floatBuffer = rawFloatBuffer;
    IntBuffer intBuffer = rawIntBuffer;
    int rbi = rawBufferIndex;
  ...
  {
    if ((wrr.func_178973_g().func_177350_b()) && (drawMode == 7) && (vertexCount % 4 == 0))
    {
      SVertexBuilder svb = sVertexBuilder;
      endSetVertexFormat(wrr);
      int indexEnd = vertexCount * vertexSize;
