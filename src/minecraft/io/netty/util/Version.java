...
public final class Version
{
    ...
    try {
      ...
      for (;;) { if (resources.hasMoreElements()) {
          ...
          try {
            ...
            try
            {
              in.close(); } catch (Exception ignore) {} } finally { try { in.close();
            }
  ...
  {
    return artifactId + '-' + artifactVersion + '.' + shortCommitHash + ("clean".equals(repositoryStatus) ? "" : new StringBuilder().append(" (repository: ").append(repositoryStatus).append(')').toString());
  }
}
