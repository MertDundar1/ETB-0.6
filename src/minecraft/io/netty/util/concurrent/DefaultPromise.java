  ...
  implements Promise<V>
{
  ...
  static { CANCELLATION_CAUSE_HOLDERcause.setStackTrace(EmptyArrays.EMPTY_STACK_TRACE); }
    ...
    synchronized (this) {
      if (!isDone()) {
        if (listeners == null) {
          ...
          GenericFutureListener<? extends Future<V>> firstListener = (GenericFutureListener)listeners;
  ...
  private void notifyLateListener(GenericFutureListener<?> l)
  {
    EventExecutor executor = executor();
    if (executor.inEventLoop()) {
      if ((listeners == null) && (this.lateListeners == null)) {
        InternalThreadLocalMap threadLocals = InternalThreadLocalMap.get();
        int stackDepth = threadLocals.futureListenerStackDepth();
        if (stackDepth < 8) {
          threadLocals.setFutureListenerStackDepth(stackDepth + 1);
  ...
  protected static void notifyListener(EventExecutor eventExecutor, Future<?> future, final GenericFutureListener<?> l)
  {
    ...
    {
      public void run() {
        DefaultPromise.notifyListener0(val$future, l);
      }
    });
  }
    ...
    implements Runnable
  {
    ...
    public void run()
    {
      ...
      DefaultPromise.execute(executor(), this);
    }
  }
