  ...
  public Set<Cookie> decode(String header)
  {
    ...
    while (i != headerLen)
    {
      ...
      else
      {
        ...
        if ((!rfc2965Style) || ((!header.regionMatches(nameBegin, "$Path", 0, "$Path".length())) && (!header.regionMatches(nameBegin, "$Domain", 0, "$Domain".length())) && (!header.regionMatches(nameBegin, "$Port", 0, "$Port".length()))))
        {
