  ...
  static int compress(byte[] input, int inOffset, int inLength, byte[] output, int outOffset, int proposedLevel)
  {
    ...
    while (ip < ipLimit) {
      ...
      if (!matchLabel)
      {
        ...
        if ((distance != 0L) && (level == 1 ? distance < 8191L : distance < 73725L)) { if ((input[(inOffset + ref++)] == input[(inOffset + ip++)]) && (input[(inOffset + ref++)] == input[(inOffset + ip++)]) && (input[(inOffset + ref++)] == input[(inOffset + ip++)])) {}
      ...
      if (ctrl >= 32L) {
        ...
        if (len == 6L) {
          if (level == 1)
          {
            ...
            int code;
            do { code = input[(inOffset + ip++)] & 0xFF;
              len += code;
            } while (code == 255);
          }
        }
