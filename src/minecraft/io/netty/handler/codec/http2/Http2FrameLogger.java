  ...
  extends ChannelHandlerAdapter
{
  ...
  {
    if (enabled()) {
      log(direction, "%s DATA: streamId=%d, padding=%d, endStream=%b, length=%d, bytes=%s", new Object[] { ctx.channel(), Integer.valueOf(streamId), Integer.valueOf(padding), Boolean.valueOf(endStream), Integer.valueOf(data.readableBytes()), toString(data) });
    }
  }
  ...
  {
    if (enabled()) {
      log(direction, "%s HEADERS: streamId=%d, headers=%s, padding=%d, endStream=%b", new Object[] { ctx.channel(), Integer.valueOf(streamId), headers, Integer.valueOf(padding), Boolean.valueOf(endStream) });
    }
  }
  ...
  {
    if (enabled()) {
      log(direction, "%s HEADERS: streamId=%d, headers=%s, streamDependency=%d, weight=%d, exclusive=%b, padding=%d, endStream=%b", new Object[] { ctx.channel(), Integer.valueOf(streamId), headers, Integer.valueOf(streamDependency), Short.valueOf(weight), Boolean.valueOf(exclusive), Integer.valueOf(padding), Boolean.valueOf(endStream) });
    }
  }
  ...
  {
    if (enabled()) {
      log(direction, "%s PRIORITY: streamId=%d, streamDependency=%d, weight=%d, exclusive=%b", new Object[] { ctx.channel(), Integer.valueOf(streamId), Integer.valueOf(streamDependency), Short.valueOf(weight), Boolean.valueOf(exclusive) });
    }
  }
  ...
  {
    if (enabled()) {
      log(direction, "%s RST_STREAM: streamId=%d, errorCode=%d", new Object[] { ctx.channel(), Integer.valueOf(streamId), Long.valueOf(errorCode) });
    }
  }
  
  public void logSettingsAck(Direction direction, ChannelHandlerContext ctx) {
    if (enabled()) {
      log(direction, "%s SETTINGS: ack=true", new Object[] { ctx.channel() });
    }
  }
  
  public void logSettings(Direction direction, ChannelHandlerContext ctx, Http2Settings settings) {
    if (enabled()) {
      log(direction, "%s SETTINGS: ack=false, settings=%s", new Object[] { ctx.channel(), settings });
    }
  }
  
  public void logPing(Direction direction, ChannelHandlerContext ctx, ByteBuf data) {
    if (enabled()) {
      log(direction, "%s PING: ack=false, length=%d, bytes=%s", new Object[] { ctx.channel(), Integer.valueOf(data.readableBytes()), toString(data) });
    }
  }
  ...
  {
    if (enabled()) {
      log(direction, "%s PING: ack=true, length=%d, bytes=%s", new Object[] { ctx.channel(), Integer.valueOf(data.readableBytes()), toString(data) });
    }
  }
  ...
  {
    if (enabled()) {
      log(direction, "%s PUSH_PROMISE: streamId=%d, promisedStreamId=%d, headers=%s, padding=%d", new Object[] { ctx.channel(), Integer.valueOf(streamId), Integer.valueOf(promisedStreamId), headers, Integer.valueOf(padding) });
    }
  }
  ...
  {
    if (enabled()) {
      log(direction, "%s GO_AWAY: lastStreamId=%d, errorCode=%d, length=%d, bytes=%s", new Object[] { ctx.channel(), Integer.valueOf(lastStreamId), Long.valueOf(errorCode), Integer.valueOf(debugData.readableBytes()), toString(debugData) });
    }
  }
  ...
  {
    if (enabled()) {
      log(direction, "%s WINDOW_UPDATE: streamId=%d, windowSizeIncrement=%d", new Object[] { ctx.channel(), Integer.valueOf(streamId), Integer.valueOf(windowSizeIncrement) });
    }
  }
  ...
  {
    if (enabled()) {
      log(direction, "%s UNKNOWN: frameType=%d, streamId=%d, flags=%d, length=%d, bytes=%s", new Object[] { ctx.channel(), Integer.valueOf(frameType & 0xFF), Integer.valueOf(streamId), Short.valueOf(flags.value()), Integer.valueOf(data.readableBytes()), toString(data) });
    }
  }
