  ...
  private static final class Http2Decompressor
  {
      ...
      throws Http2Exception
    {
      ...
      if (decompressed - decompressedBytes < 0) {
        throw Http2Exception.streamError(streamId, Http2Error.INTERNAL_ERROR, "Attempting to return too many bytes for stream %d. decompressed: %d decompressedBytes: %d", new Object[] { Integer.valueOf(streamId), Integer.valueOf(decompressed), Integer.valueOf(decompressedBytes) });
      }
      ...
      if (compressed - consumedCompressed < 0) {
        throw Http2Exception.streamError(streamId, Http2Error.INTERNAL_ERROR, "overflow when converting decompressed bytes to compressed bytes for stream %d.decompressedBytes: %d decompressed: %d compressed: %d consumedCompressed: %d", new Object[] { Integer.valueOf(streamId), Integer.valueOf(decompressedBytes), Integer.valueOf(decompressed), Integer.valueOf(compressed), Integer.valueOf(consumedCompressed) });
      }
