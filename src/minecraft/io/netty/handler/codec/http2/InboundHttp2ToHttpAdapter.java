  ...
  extends Http2EventAdapter
{
  ...
  {
    return connection.isServer() ? HttpConversionUtil.toFullHttpRequest(stream.id(), headers, alloc, validateHttpHeaders) : HttpConversionUtil.toHttpResponse(stream.id(), headers, alloc, validateHttpHeaders);
  }
    ...
    throws Http2Exception
  {
    ...
    if (sendDetector.mustSendImmediately(msg))
    {
      ...
      FullHttpMessage copy = endOfStream ? null : sendDetector.copyIfNeeded(msg);
    ...
    throws Http2Exception
  {
    ...
    if (content.readableBytes() > maxContentLength - dataReadableBytes) {
      throw Http2Exception.connectionError(Http2Error.INTERNAL_ERROR, "Content length exceeded max of %d for stream id %d", new Object[] { Integer.valueOf(maxContentLength), Integer.valueOf(streamId) });
    }
