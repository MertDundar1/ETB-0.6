  ...
  implements Http2LocalFlowController
{
    ...
    implements DefaultHttp2LocalFlowController.FlowState
  {
    ...
    {
      int newValue = (int)Math.min(2147483647L, Math.max(0L, initialStreamWindowSize + delta));
    ...
    public void incrementFlowControlWindows(int delta) throws Http2Exception
    {
      if ((delta > 0) && (window > Integer.MAX_VALUE - delta)) {
        throw Http2Exception.streamError(stream.id(), Http2Error.FLOW_CONTROL_ERROR, "Flow control window overflowed for stream: %d", new Object[] { Integer.valueOf(stream.id()) });
      }
    ...
    public void receiveFlowControlledFrame(int dataLength) throws Http2Exception
    {
      ...
      if (window < lowerBound) {
        throw Http2Exception.streamError(stream.id(), Http2Error.FLOW_CONTROL_ERROR, "Flow control window exceeded for stream: %d", new Object[] { Integer.valueOf(stream.id()) });
      }
    }
    ...
    private void returnProcessedBytes(int delta) throws Http2Exception
    {
      if (processedWindow - delta < window) {
        throw Http2Exception.streamError(stream.id(), Http2Error.INTERNAL_ERROR, "Attempting to return too many bytes for stream %d", new Object[] { Integer.valueOf(stream.id()) });
      }
      ...
      throws Http2Exception
    {
      ...
      try {
        ...
        throw Http2Exception.connectionError(Http2Error.INTERNAL_ERROR, t, "Attempting to return too many bytes for stream %d", new Object[] { Integer.valueOf(stream.id()) });
      }
