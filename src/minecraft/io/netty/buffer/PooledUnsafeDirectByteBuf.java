  ...
  extends PooledByteBuf<ByteBuffer>
{
  ...
  private static final Recycler<PooledUnsafeDirectByteBuf> RECYCLER = new Recycler()
  {
    ...
    protected PooledUnsafeDirectByteBuf newObject(Recycler.Handle handle) { return new PooledUnsafeDirectByteBuf(handle, 0, null); }
  };
  ...
  private void getBytes(int index, ByteBuffer dst, boolean internal) {
    ...
    ByteBuffer tmpBuf; if (internal) {
      ...
      tmpBuf = ((ByteBuffer)memory).duplicate();
    }
    index = idx(index);
  ...
  public ByteBuffer nioBuffer(int index, int length)
  {
    checkIndex(index, length);
    index = idx(index);
    return ((ByteBuffer)((ByteBuffer)memory).duplicate().position(index).limit(index + length)).slice();
  }
