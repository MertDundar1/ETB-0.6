...
public class SoundSystemConfig
{
    ...
    throws SoundSystemException
  {
    ...
    while (i.hasNext())
    {
      ...
      if (extension.matches(extensionRegX))
        i.remove();
    }
  ...
  public static synchronized ICodec getCodec(String filename)
  {
    ...
    {
      Codec codec = (Codec)i.next();
      if (filename.matches(extensionRegX)) {
        ...
        synchronized (SoundSystemConfig.streamListenersLock)
        {
          ...
          while (i.hasNext())
          {
            ...
            if (streamListener == null) {
              ...
              streamListener.endOfStream(val$srcName, qSize);
            }
          }
        }
      }
    ...
    public Codec(String extension, Class iCodecClass)
    {
      ...
      if ((extension != null) && (extension.length() > 0))
      {
        ...
        for (int x = 0; x < extension.length(); x++)
        {
          ...
          extensionRegX = (extensionRegX + "[" + c.toLowerCase(Locale.ENGLISH) + c.toUpperCase(Locale.ENGLISH) + "]");
        }
