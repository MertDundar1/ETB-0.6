  ...
  extends AbstractSetMultimap<K, V>
{
    ...
    implements LinkedHashMultimap.ValueSetLink<K, V>
  {
    ...
    public boolean contains(@Nullable Object o)
    {
      ...
      for (LinkedHashMultimap.ValueEntry<K, V> entry = hashTable[(smearedHash & mask())]; entry != null; 
    ...
    private void rehashIfNecessary() {
      if (Hashing.needsResizing(size, this.hashTable.length, 1.0D))
      {
        ...
        for (LinkedHashMultimap.ValueSetLink<K, V> entry = firstEntry; 
    ...
    public boolean remove(@Nullable Object o)
    {
      ...
      for (LinkedHashMultimap.ValueEntry<K, V> entry = hashTable[bucket]; entry != null; 
          entry = nextInValueBucket) {
        if (entry.matchesValue(o, smearedHash)) {
          if (prev == null)
          {
