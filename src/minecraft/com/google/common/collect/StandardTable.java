  ...
  implements Serializable
{
  ...
  private class CellIterator implements Iterator<Table.Cell<R, C, V>> {
    ...
    public boolean hasNext() { return (rowIterator.hasNext()) || (columnIterator.hasNext()); }
  ...
  class Row extends Maps.ImprovedAbstractMap<C, V> {
    ...
    Row() { this.rowKey = Preconditions.checkNotNull(rowKey); }
    


    Map<C, V> backingRowMap()
    {
      return (backingRowMap == null) || ((backingRowMap.isEmpty()) && (backingMap.containsKey(rowKey))) ? (this.backingRowMap = computeBackingRowMap()) : backingRowMap;
    }
    ...
    private final class RowEntrySet extends Maps.EntrySet<C, V> {
      ...
      Map<C, V> map() { return StandardTable.Row.this; }
        ...
        new Iterator()
        {
          ...
          public Map.Entry<C, V> next() {
            ...
            new ForwardingMapEntry() {
              ...
              public V setValue(V value) { return super.setValue(Preconditions.checkNotNull(value)); }
  ...
  private class Column extends Maps.ImprovedAbstractMap<R, V> {
    ...
    public V put(R key, V value) {
      return put(key, columnKey, value);
    }
    
    public V get(Object key) {
      return get(key, columnKey);
    }
    ...
    Set<Map.Entry<R, V>> createEntrySet() { return new EntrySet(null); }
    
    private class EntrySet extends Sets.ImprovedAbstractSet<Map.Entry<R, V>> {
      private EntrySet() {}
      
      public Iterator<Map.Entry<R, V>> iterator() { return new StandardTable.Column.EntrySetIterator(StandardTable.Column.this, null); }
      ...
      public boolean retainAll(Collection<?> c) { return removeFromColumnIf(Predicates.not(Predicates.in(c))); }
    }
    
    private class EntrySetIterator extends AbstractIterator<Map.Entry<R, V>> { private EntrySetIterator() {}
        ...
        while (iterator.hasNext()) {
          ...
          if (((Map)entry.getValue()).containsKey(columnKey)) {
            new AbstractMapEntry() {
              ...
              public V getValue() { return ((Map)entry.getValue()).get(columnKey); }
              
              public V setValue(V value) {
                return ((Map)entry.getValue()).put(columnKey, Preconditions.checkNotNull(value));
              }
            };
          }
        }
  ...
  private class ColumnKeySet extends StandardTable<R, C, V>.TableSet<C> { private ColumnKeySet() { super(null); }
    
    public Iterator<C> iterator() { return createColumnKeyIterator(); }
    ...
    extends AbstractIterator<C>
  {
    ...
    protected C computeNext() { for (;;) { if (entryIterator.hasNext()) {
          Map.Entry<C, V> entry = (Map.Entry)entryIterator.next();
          if (!seen.containsKey(entry.getKey())) {
            seen.put(entry.getKey(), entry.getValue());
            return entry.getKey();
          }
        } else { if (!mapIterator.hasNext()) break;
  ...
  class RowMap extends Maps.ImprovedAbstractMap<R, Map<C, V>> {
    ...
    public boolean containsKey(Object key) { return containsRow(key); }
    ...
    protected Set<Map.Entry<R, Map<C, V>>> createEntrySet() { return new EntrySet(); }
    
    class EntrySet extends StandardTable<R, C, V>.TableSet<Map.Entry<R, Map<C, V>>> {
      EntrySet() { super(null); }
      
      public Iterator<Map.Entry<R, Map<C, V>>> iterator() { Maps.asMapEntryIterator(backingMap.keySet(), new Function()
        {
  ...
  private class ColumnMap extends Maps.ImprovedAbstractMap<C, Map<R, V>>
  {
    ...
    Collection<Map<R, V>> createValues() { return new ColumnMapValues(); }
    
    class ColumnMapEntrySet extends StandardTable<R, C, V>.TableSet<Map.Entry<C, Map<R, V>>> {
      ColumnMapEntrySet() { super(null); }
      
      public Iterator<Map.Entry<C, Map<R, V>>> iterator() { Maps.asMapEntryIterator(columnKeySet(), new Function()
        {
