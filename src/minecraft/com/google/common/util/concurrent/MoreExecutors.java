...
public final class MoreExecutors
{
  ...
  static class Application {
    ...
    @VisibleForTesting
    void addShutdownHook(Thread hook) { Runtime.getRuntime().addShutdownHook(hook); }
  }
    ...
    extends AbstractListeningExecutorService
  {
    ...
    public void execute(Runnable command) { startTask();
      try {
        command.run();
      } finally {
        endTask();
  ...
  private static <T> ListenableFuture<T> submitAndAddQueueListener(ListeningExecutorService executorService, Callable<T> task, BlockingQueue<Future<T>> queue)
  {
    ...
    {
      public void run() { val$queue.add(future); } }, sameThreadExecutor());
  ...
  static ExecutorService renamingDecorator(ExecutorService service, final Supplier<String> nameSupplier)
  {
    ...
    new WrappingExecutorService(service) {
      ...
      protected Runnable wrapTask(Runnable command) { return Callables.threadRenaming(command, nameSupplier); }
    };
  }
