...
class DecodeExample
{
  ...
  public static void main(String[] arg) {
    ...
    for (;;)
    {
      ...
      byte[][] ptr = user_comments;
      for (int j = 0; j < ptr.length; j++) {
        if (ptr[j] == null)
          break;
        System.err.println(new String(ptr[j], 0, ptr[j].length - 1));
      }
      System.err.println("\nBitstream is " + channels + " channel, " + rate + "Hz");
      
      System.err.println("Encoded by: " + new String(vendor, 0, vendor.length - 1) + "\n");
            ...
            for (;;)
            {
              ...
              if (result != -1)
              {
                ...
                while ((samples = vd.synthesis_pcmout(_pcm, _index)) > 0) {
                  ...
                  System.out.write(convbuffer, 0, 2 * channels * bout);
