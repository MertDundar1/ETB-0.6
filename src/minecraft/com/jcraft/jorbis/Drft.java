...
class Drft
{
  ...
  static void fdrffti(int n, float[] wsave, int[] ifac) { if (n == 1)
      return;
  ...
  static void dradf4(int ido, int l1, float[] cc, float[] ch, float[] wa1, int index1, float[] wa2, int index2, float[] wa3, int index3)
  {
    ...
    for (int k = 0; k < l1; k++) {
      ...
      float tr2 = cc[t3] + cc[t4]; int 
      
        tmp63_62 = (t3 << 2);int t5 = tmp63_62;ch[tmp63_62] = (tr1 + tr2);
      ch[((ido << 2) + t5 - 1)] = (tr2 - tr1); int 
        tmp94_93 = (t5 + (ido << 1));t5 = tmp94_93;ch[(tmp94_93 - 1)] = (cc[t3] - cc[t4]);
      cc[t2] -= cc[t1];
  ...
  static void dradfg(int ido, int ip, int l1, int idl1, float[] cc, float[] c1, float[] c2, float[] ch, float[] ch2, float[] wa, int index)
  {
    ...
    int state = 100;
    for (;;) { int ik;
      int t1; int j; int t3; int k; int i; int t1; int t4; int t5; switch (state) {
      case 101: 
        if (ido == 1) {
          state = 119;
        }
  ...
  static void dradb4(int ido, int l1, float[] cc, float[] ch, float[] wa1, int index1, float[] wa2, int index2, float[] wa3, int index3)
  {
    ...
    for (int k = 0; k < l1; k++) {
      ...
      ch[t5] = (tr2 + tr3); int 
        tmp114_113 = (t5 + t0);t5 = tmp114_113;ch[tmp114_113] = (tr1 - tr4); int 
        tmp129_128 = (t5 + t0);t5 = tmp129_128;ch[tmp129_128] = (tr2 - tr3); int 
        tmp144_143 = (t5 + t0);t5 = tmp144_143;ch[tmp144_143] = (tr1 + tr4);
      t1 += ido;
      t3 += t2;
    }
    ...
    if (ido != 2) {
      ...
      for (k = 0; k < l1; k++) { int t4;
        ...
        for (int i = 2; i < ido; i += 2) {
          ...
          float cr4 = tr1 + tr4;
          float ci2 = ti1 + ti4;
          float ci4 = ti1 - ti4; int 
          
            tmp416_415 = (t7 + t0);int t8 = tmp416_415;ch[(tmp416_415 - 1)] = (wa1[(index1 + i - 2)] * cr2 - wa1[(index1 + i - 1)] * ci2);
          ch[t8] = (wa1[(index1 + i - 2)] * ci2 + wa1[(index1 + i - 1)] * cr2); int 
            tmp486_485 = (t8 + t0);t8 = tmp486_485;ch[(tmp486_485 - 1)] = (wa2[(index2 + i - 2)] * cr3 - wa2[(index2 + i - 1)] * ci3);
          ch[t8] = (wa2[(index2 + i - 2)] * ci3 + wa2[(index2 + i - 1)] * cr3); int 
            tmp556_555 = (t8 + t0);t8 = tmp556_555;ch[(tmp556_555 - 1)] = (wa3[(index3 + i - 2)] * cr4 - wa3[(index3 + i - 1)] * ci4);
    ...
    for (k = 0; k < l1; k++) {
      ...
      ch[t5] = (tr2 + tr2); int 
        tmp746_745 = (t5 + t0);t5 = tmp746_745;ch[tmp746_745] = (sqrt2 * (tr1 - ti1)); int 
        tmp765_764 = (t5 + t0);t5 = tmp765_764;ch[tmp765_764] = (ti2 + ti2); int 
        tmp780_779 = (t5 + t0);t5 = tmp780_779;ch[tmp780_779] = (-sqrt2 * (tr1 + ti1));
  ...
  static void dradbg(int ido, int ip, int l1, int idl1, float[] cc, float[] c1, float[] c2, float[] ch, float[] ch2, float[] wa, int index)
  {
    ...
    for (;;) { int t1;
      int t2;
      int k; int i; int t2; int t5; int t7; int j; int t3; int j; int is; switch (state) {
      case 100: 
        t10 = ip * ido;
        t0 = l1 * ido;
        float arg = tpi / ip;
