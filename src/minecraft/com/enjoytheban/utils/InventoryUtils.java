...
public class InventoryUtils
{
  ...
  public void dropSlot(int slot) {
    int windowId = net.minecraft.client.gui.inventory.GuiInventorymcthePlayer).inventorySlots.windowId;
    mcplayerController.windowClick(windowId, slot, 1, 4, mcthePlayer);
  }
  ...
  public static void updateInventory() {
    for (int index = 0; index < 44; index++) {
      try {
        ...
        getMinecraftthePlayer.sendQueue.addToSendQueue(new net.minecraft.network.play.client.C10PacketCreativeInventoryAction(index + offset, getMinecraftthePlayer.inventory.mainInventory[index]));
      }
  ...
  public static ItemStack getStackInSlot(int slot) {
    return mcthePlayer.inventory.getStackInSlot(slot);
  }
  ...
  public static boolean isBestArmorOfTypeInInv(ItemStack is) {
    try {
      ...
      for (int i = 0; i < 4; i++) {
        ItemStack stack = mcthePlayer.inventory.armorInventory[i];
        if (stack != null) {
          ItemArmor otherArmor = (ItemArmor)stack.getItem();
          if (armorType == armorType) {
            int otherProt = getArmorProt(stack);
            if (otherProt >= prot) {
              return false;
            }
          }
      ...
      for (i = 0; i < mcthePlayer.inventory.getSizeInventory() - 4; i++) {
        ItemStack stack = mcthePlayer.inventory.getStackInSlot(i);
        if ((stack != null) && ((stack.getItem() instanceof ItemArmor))) {
          ItemArmor otherArmor = (ItemArmor)stack.getItem();
          if ((armorType == armorType) && (otherArmor != ia)) {
            int otherProt = getArmorProt(stack);
            if (otherProt >= prot) {
              return false;
            }
  ...
  public static boolean hotbarHas(Item item) {
    for (int index = 0; index <= 36; index++) {
      ItemStack stack = getMinecraftthePlayer.inventory.getStackInSlot(index);
  ...
  private static double getSwordDamage(ItemStack itemStack) {
    ...
    java.util.Optional<AttributeModifier> attributeModifier = itemStack.getAttributeModifiers().values().stream().findFirst();
    
    if (attributeModifier.isPresent()) {
      damage = ((AttributeModifier)attributeModifier.get()).getAmount();
    }
    
    damage += EnchantmentHelper.func_152377_a(itemStack, net.minecraft.entity.EnumCreatureAttribute.UNDEFINED);
  ...
  public boolean isBestChest(int slot)
  {
    if ((getStackInSlot(slot) != null) && (getStackInSlot(slot).getItem() != null) && 
      ...
      int slotProtection = ((ItemArmor)mcthePlayer.inventory.getStackInSlot(slot).getItem()).getArmorMaterial()
        .getDamageReductionAmount(getItemType(mcthePlayer.inventory.getStackInSlot(slot))) + 
        EnchantmentHelper.getEnchantmentModifierDamage(
        new ItemStack[] { mcthePlayer.inventory.getStackInSlot(slot) }, DamageSource.generic);
      
      if (mcthePlayer.inventory.armorInventory[2] != null) {
        ItemArmor ia = (ItemArmor)mcthePlayer.inventory.armorInventory[2].getItem();
        ItemStack is = mcthePlayer.inventory.armorInventory[2];
      ...
      for (int i = 0; i < mcthePlayer.inventory.getSizeInventory(); i++) {
        if (getStackInSlot(i) != null)
        {

          if ((mcthePlayer.inventory.getStackInSlot(i).getItem() instanceof ItemArmor)) {
            int otherProtection = ((ItemArmor)mcthePlayer.inventory.getStackInSlot(i).getItem())
              .getArmorMaterial()
              .getDamageReductionAmount(getItemType(mcthePlayer.inventory.getStackInSlot(i))) + 
              EnchantmentHelper.getEnchantmentModifierDamage(
              new ItemStack[] { mcthePlayer.inventory.getStackInSlot(i) }, DamageSource.generic);
            
            ItemArmor ia1 = (ItemArmor)getStackInSlot(slot).getItem();
            
            ItemArmor ia2 = (ItemArmor)getStackInSlot(i).getItem();
            
            if ((armorType == 1) && (armorType == 1) && 
              (otherProtection > slotProtection)) {
              return false;
            }
          }
        }
  ...
  public boolean isBestHelmet(int slot) {
    if ((getStackInSlot(slot) != null) && (getStackInSlot(slot).getItem() != null) && 
      ...
      if (mcthePlayer.inventory.armorInventory[3] != null) {
        ItemArmor ia = (ItemArmor)mcthePlayer.inventory.armorInventory[3].getItem();
        ItemStack is = mcthePlayer.inventory.armorInventory[3];
  ...
  public boolean isBestLeggings(int slot)
  {
    if ((getStackInSlot(slot) != null) && (getStackInSlot(slot).getItem() != null) && 
      ...
      int slotProtection = ((ItemArmor)mcthePlayer.inventory.getStackInSlot(slot).getItem()).getArmorMaterial()
        ...
        EnchantmentHelper.getEnchantmentModifierDamage(
        new ItemStack[] { mcthePlayer.inventory.getStackInSlot(slot) }, DamageSource.generic);
      if (mcthePlayer.inventory.armorInventory[1] != null) {
        ItemArmor ia = (ItemArmor)mcthePlayer.inventory.armorInventory[1].getItem();
        ItemStack is = mcthePlayer.inventory.armorInventory[1];
  ...
  public boolean isBestBoots(int slot)
  {
    if ((getStackInSlot(slot) != null) && (getStackInSlot(slot).getItem() != null) && 
      ...
      if (mcthePlayer.inventory.armorInventory[0] != null) {
        ItemArmor ia = (ItemArmor)mcthePlayer.inventory.armorInventory[0].getItem();
        ItemStack is = mcthePlayer.inventory.armorInventory[0];
  ...
  public int getBestWeaponInHotbar() {
    int originalSlot = mcthePlayer.inventory.currentItem;
    byte weaponSlot = -1;
    float weaponDamage = 1.0F;
    
    for (byte slot = 0; slot < 9; slot = (byte)(slot + 1)) {
      ItemStack itemStack = mcthePlayer.inventory.getStackInSlot(slot);
      if (itemStack != null) {
        float damage = getItemDamage(itemStack);
        
        if (damage += EnchantmentHelper.func_152377_a(itemStack, net.minecraft.entity.EnumCreatureAttribute.UNDEFINED) > weaponDamage) {
          weaponDamage = damage;
          weaponSlot = slot;
        }
      }
  ...
  public int getBestWeapon()
  {
    ...
    for (byte slot = 0; slot < mcthePlayer.inventory.getSizeInventory(); slot = (byte)(slot + 1))
    {
  ...
  public int getArmorProt(int i)
  {
    ...
    if ((getStackInSlot(i) != null) && (getStackInSlot(i).getItem() != null) && 
      ...
      armorprot = ((ItemArmor)mcthePlayer.inventory.getStackInSlot(i).getItem()).getArmorMaterial()
        .getDamageReductionAmount(getItemType(mcthePlayer.inventory.getStackInSlot(i))) + 
        EnchantmentHelper.getEnchantmentModifierDamage(
        new ItemStack[] { mcthePlayer.inventory.getStackInSlot(i) }, DamageSource.generic);
    }
  ...
  public static boolean isBestSword(ItemStack itemSword, int slot) {
    if ((itemSword != null) && ((itemSword.getItem() instanceof ItemSword))) {
      for (int i = 0; i < mcthePlayer.inventory.getSizeInventory(); i++) {
        ItemStack iStack = mcthePlayer.inventory.getStackInSlot(i);
