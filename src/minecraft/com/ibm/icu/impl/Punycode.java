...
public final class Punycode
{
    ...
    throws StringPrepParseException
  {
    ...
    for (int j = 0; j < srcLength; j++) {
      ...
      if (isBasic(c)) {
        ...
        int n = ((caseFlags != null) && (caseFlags[j] != 0) ? 1 : 0) << 31;
    ...
    for (int handledCPCount = basicLength; handledCPCount < srcCPCount;)
    {
      ...
      int m = Integer.MAX_VALUE; for (j = 0; j < srcCPCount; j++) {
        int q = cpBuffer[j] & 0x7FFFFFFF;
        if ((n <= q) && (q < m)) {
          m = q;
        }
      }
      ...
      for (j = 0; j < srcCPCount; j++) {
        ...
        } else if (q == n)
        {
          ...
          if (destLength < destCapacity) {
            dest[(destLength++)] = digitToBasic(q, cpBuffer[j] < 0 ? 1 : false);
          }
    ...
    throws StringPrepParseException
  {
    ...
    for (int j = srcLength; j > 0;)
      if (src.charAt(--j) == '-')
        break;
    int destCPCount;
    int basicLength;
    int destLength = basicLength = destCPCount = j;
    ...
    for (int in = basicLength > 0 ? basicLength + 1 : 0; in < srcLength;)
    {
      ...
      int oldi = i;int w = 1; for (int k = 36;; k += 36) {
        if (in >= srcLength) {
          throw new StringPrepParseException("Illegal char found", 1);
        }
