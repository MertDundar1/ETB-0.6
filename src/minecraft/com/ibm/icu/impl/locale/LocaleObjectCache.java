...
public abstract class LocaleObjectCache<K, V>
{
  ...
  public LocaleObjectCache(int initialCapacity, float loadFactor, int concurrencyLevel) {
    _map = new ConcurrentHashMap(initialCapacity, loadFactor, concurrencyLevel);
  }
  ...
  public V get(K key) {
    ...
    cleanStaleEntries();
    CacheEntry<K, V> entry = (CacheEntry)_map.get(key);
    if (entry != null) {
      value = entry.get();
    }
    if (value == null) {
      ...
      CacheEntry<K, V> newEntry = new CacheEntry(key, newVal, _queue);
      
      while (value == null) {
        cleanStaleEntries();
        entry = (CacheEntry)_map.putIfAbsent(key, newEntry);
        if (entry == null) {
          value = newVal;
          break;
        }
