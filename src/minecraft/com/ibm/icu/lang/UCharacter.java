  ...
  implements UCharacterEnums.ECharacterCategory, UCharacterEnums.ECharacterDirection
{
    ...
    extends Character.Subset
  {
    ...
    private static final UnicodeBlock[] BLOCKS_ = new UnicodeBlock['Ý'];
    ...
    static { for (int blockId = 0; blockId < 221; blockId++) {
  ...
  {
    return getNFCInstancedecomp.getCombiningClass(ch);
  }
  ...
  {
    return INSTANCEm_unicodeVersion_;
  }
  ...
  public static String getName(String s, String separator)
  {
    ...
    int cp; for (int i = 0; i < s.length(); i += UTF16.getCharCount(cp)) {
      cp = UTF16.charAt(s, i);
  ...
  private static class StringContextIterator implements UCaseProps.ContextIterator { StringContextIterator(String s) { this.s = s;
      limit = s.length();
      cpStart = (this.cpLimit = this.index = 0);
      dir = 0;
    }
    ...
    public int nextCaseMapCP()
    {
      ...
      if (cpLimit < limit) {
        ...
        if ((55296 <= c) || (c <= 57343)) {
          char c2;
          if ((c <= 56319) && (cpLimit < limit) && (56320 <= (c2 = s.charAt(cpLimit))) && (c2 <= 57343))
          {
    ...
    public int next()
    {
      if ((dir > 0) && (index < s.length())) {
        int c = UTF16.charAt(s, index);
        index += UTF16.getCharCount(c);
        return c; }
      if ((dir < 0) && (index > 0)) {
        int c = UTF16.charAt(s, index - 1);
        index -= UTF16.getCharCount(c);
        return c;
      }
      return -1;
  ...
  public static String toTitleCase(ULocale locale, String str, BreakIterator titleIter, int options)
  {
    ...
    while (prev < srcLength) { int index;
      int index;
      if (isFirstIndex) {
        isFirstIndex = false;
      ...
      if (prev < index)
      {
        ...
        int titleStart; if (((options & 0x200) == 0) && (0 == UCaseProps.INSTANCE.getType(c)))
        {

          while (((c = iter.nextCaseMapCP()) >= 0) && (0 == UCaseProps.INSTANCE.getType(c))) {}
          int titleStart = iter.getCPStart();
          if (prev < titleStart) {
        ...
        if (titleStart < index) {
          ...
          for (;;)
          {
            ...
            if ((options & 0x100) != 0)
            {
              ...
              iter.moveToLimit();
              break; }
            int nc; if ((nc = iter.nextCaseMapCP()) < 0) break;
  ...
  public static RangeValueIterator getTypeIterator() { return new UCharacterTypeIterator(); }
  ...
  private static final class UCharacterTypeIterator implements RangeValueIterator { private Iterator<Trie2.Range> trieIterator;
    ...
    UCharacterTypeIterator() { reset(); }
    

    public boolean next(RangeValueIterator.Element element)
    {
      if ((trieIterator.hasNext()) && (!trieIterator.next()).leadSurrogate)) {
    ...
    public void reset()
    {
      trieIterator = INSTANCEm_trie_.iterator(MASK_TYPE);
    }
      ...
      implements Trie2.ValueMapper
    {
      ...
      public int map(int value) { return value & 0x1F; }
    }
   ...
   * @deprecated
   */
  public static ValueIterator getName1_0Iterator() { return new DummyValueIterator(null); }
  
  private static final class DummyValueIterator implements ValueIterator { private DummyValueIterator() {}
    
    public boolean next(ValueIterator.Element element) { return false; }
  ...
  public static String getStringPropertyValue(int propertyEnum, int codepoint, int nameChoice)
  {
    ...
    switch (propertyEnum) {
    case 16384:  return getAge(codepoint).toString();
    case 16387:  return getISOComment(codepoint);
    case 16385:  return UTF16.valueOf(getMirror(codepoint));
    case 16386:  return foldCase(UTF16.valueOf(codepoint), true);
    case 16388:  return toLowerCase(UTF16.valueOf(codepoint));
    case 16389:  return getName(codepoint);
    case 16390:  return UTF16.valueOf(foldCase(codepoint, true));
    case 16391:  return UTF16.valueOf(toLowerCase(codepoint));
    case 16392:  return UTF16.valueOf(toTitleCase(codepoint));
    case 16393:  return UTF16.valueOf(toUpperCase(codepoint));
    case 16394:  return toTitleCase(UTF16.valueOf(codepoint), null);
    case 16395:  return getName1_0(codepoint);
    case 16396:  return toUpperCase(UTF16.valueOf(codepoint));
    }
  ...
  public static int codePointCount(CharSequence text, int start, int limit)
  {
    ...
    while (limit > start) {
      ...
      while ((ch >= 56320) && (ch <= 57343) && (limit > start)) {
  ...
  public static int offsetByCodePoints(CharSequence text, int index, int codePointOffset)
  {
    ...
    if (codePointOffset < 0) {
      for (;;) { codePointOffset++; if (codePointOffset > 0) break;
        char ch = text.charAt(--index);
        while ((ch >= 56320) && (ch <= 57343) && (index > 0)) {
          ch = text.charAt(--index);
          if ((ch < 55296) || (ch > 56319)) {
            codePointOffset++; if (codePointOffset > 0) {
              return index + 1;
            }
          }
        }
    ...
    for (;;) { codePointOffset--; if (codePointOffset < 0) break;
      char ch = text.charAt(index++);
      while ((ch >= 55296) && (ch <= 56319) && (index < limit)) {
        ch = text.charAt(index++);
  ...
  public static int offsetByCodePoints(char[] text, int start, int count, int index, int codePointOffset)
  {
    ...
    for (;;) {
      codePointOffset--; if (codePointOffset < 0) break;
      char ch = text[(index++)];
      if (index > limit) {
