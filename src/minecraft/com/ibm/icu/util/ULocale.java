  ...
  implements Serializable
{
  ...
  {
    localeID = getName(forLocale(loc).toString());
    locale = loc;
  }
  ...
  {
    localeID = getName(lscvToID(a, b, c, ""));
  }
  ...
  public boolean equals(Object obj)
  {
    ...
    if ((obj instanceof ULocale)) {
      return localeID.equals(localeID);
    }
    return false;
  }
  ...
  public static String getName(String localeID)
  {
    ...
    if ((localeID != null) && (!localeID.contains("@")) && (getShortestSubtagLength(localeID) == 1)) {
      String tmpLocaleID = forLanguageTag(localeID).getName();
  ...
  public static String canonicalize(String localeID)
  {
    ...
    for (int i = 0; i < CANONICALIZE_MAP.length; i++) {
      if (CANONICALIZE_MAP[i][0].equals(baseName)) {
        ...
        parser.defaultKeywordValue(vals[2], vals[3]); break;
      }
    }
    ...
    if ((!foundVariant) && 
      (parser.getLanguage().equals("nb")) && (parser.getVariant().equals("NY"))) {
  ...
  {
    TreeMap<1ULocaleAcceptLanguageQ, ULocale> map = new TreeMap();
    ...
    boolean q1 = false;
    for (int n = 0; n < acceptLanguage.length(); n++) {
      ...
      if (gotLanguageQ) {
        ...
        if (languageRangeBuf.charAt(0) != '*') {
          int serial = map.size();
          Comparable entry = new Comparable()
          {
            private double q;
  ...
  public static ULocale addLikelySubtags(ULocale loc)
  {
    ...
    int trailingIndex = parseTagString(localeID, tags);
    


    if (trailingIndex < localeID.length()) {
      trailing = localeID.substring(trailingIndex);
    }
  ...
  public static ULocale minimizeSubtags(ULocale loc)
  {
    ...
    if (trailingIndex < localeID.length())
    {
      ...
      originalTrailing = localeID.substring(trailingIndex);
    }
    ...
    try {
      ...
      for (int i = 0; i < typeMapForKey.getSize(); i++) {
        ...
        if (bcpType.equals(mapData.getString())) {
          type = mapData.getKey();
          if (!key.equals("timezone")) break;
          type = type.replace(':', '/'); break;
        }
      }
    }
    catch (MissingResourceException mre) {}
  ...
  private static final class JDKLocaleHelper
  {
    ...
    private static final String[][] JAVA6_MAPDATA = { { "ja_JP_JP", "ja_JP", "calendar", "japanese", "ja" }, { "no_NO_NY", "nn_NO", null, null, "nn" }, { "th_TH_TH", "th_TH", "numbers", "thai", "th" } };
    ...
    public static Locale getDefault(ULocale.Category category) {
      ...
      if (isJava7orNewer) {
        Object cat = null;
        switch (ULocale.1.$SwitchMap$com$ibm$icu$util$ULocale$Category[category.ordinal()]) {
        case 1: 
          cat = eDISPLAY;
          break;
        case 2: 
      ...
      if (System.getSecurityManager() != null) {
        try {
          val = (String)AccessController.doPrivileged(new PrivilegedAction() {
            public String run() {
              return System.getProperty(val$fkey);
            }
