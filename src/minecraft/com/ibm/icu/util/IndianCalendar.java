  ...
  extends Calendar
{
  ...
  protected void handleComputeFields(int julianDay)
  {
    ...
    {
      IndianYear--;
      int leapMonth = isGregorianLeap(gregorianDay[0] - 1) ? 31 : 30;
    ...
    int IndianMonth;
    int IndianDayOfMonth; if (yday < leapMonth) {
      int IndianMonth = 0;
      IndianDayOfMonth = yday + 1;
    } else {
      int mday = yday - leapMonth;
      int IndianDayOfMonth; if (mday < 155) {
        int IndianMonth = mday / 31 + 1;
  ...
  private static final int[][] LIMITS = { { 0, 0, 0, 0 }, { -5000000, -5000000, 5000000, 5000000 }, { 0, 0, 11, 11 }, { 1, 1, 52, 53 }, new int[0], { 1, 1, 30, 31 }, { 1, 1, 365, 366 }, new int[0], { -1, -1, 5, 5 }, new int[0], new int[0], new int[0], new int[0], new int[0], new int[0], new int[0], new int[0], { -5000000, -5000000, 5000000, 5000000 }, new int[0], { -5000000, -5000000, 5000000, 5000000 }, new int[0], new int[0] };
  ...
  private static double IndianToJD(int year, int month, int date)
  {
    ...
    double start;
    int leapMonth;
    double start; if (isGregorianLeap(gyear)) {
      int leapMonth = 31;
  ...
  private static double gregorianToJD(int year, int month, int date)
  {
    ...
    int y = year - 1;
    int result = 365 * y + y / 4 - y / 100 + y / 400 + (367 * month - 362) / 12 + (isGregorianLeap(year) ? -1 : month <= 2 ? 0 : -2) + date;
  ...
  private static int[] jdToGregorian(double jd)
  {
    ...
    double leapadj = isGregorianLeap(year) ? 1 : wjd < gregorianToJD(year, 3, 1) ? 0 : 2;
