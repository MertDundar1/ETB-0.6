  ...
  extends Calendar
{
  ...
  private static final int[][] LIMITS = { { 0, 0, 0, 0 }, { 1, 1, 5000000, 5000000 }, { 0, 0, 11, 11 }, { 1, 1, 50, 51 }, new int[0], { 1, 1, 29, 30 }, { 1, 1, 354, 355 }, new int[0], { -1, -1, 5, 5 }, new int[0], new int[0], new int[0], new int[0], new int[0], new int[0], new int[0], new int[0], { 1, 1, 5000000, 5000000 }, new int[0], { 1, 1, 5000000, 5000000 }, new int[0], new int[0] };
  ...
  private long monthStart(int year, int month)
  {
    ...
    if (civil) {
      return Math.ceil(29.5D * realMonth) + (realYear - 1) * 354 + Math.floor((3 + 11 * realYear) / 30.0D);
    }
  ...
  private static final long trueMonthStart(long month)
  {
    ...
    if (start == CalendarCache.EMPTY)
    {
      ...
      long origin = -42521587200000L + Math.floor(month * 29.530588853D) * 86400000L;
  ...
  protected void handleComputeFields(int julianDay)
  {
    ...
    int month; if (civil)
    {
      int year = (int)Math.floor((30L * days + 10646L) / 10631.0D);
      int month = (int)Math.ceil((days - 29L - yearStart(year)) / 29.5D);
      month = Math.min(month, 11);
    }
    else {
      int months = (int)Math.floor(days / 29.530588853D);
      
      long monthStart = Math.floor(months * 29.530588853D - 1.0D);
