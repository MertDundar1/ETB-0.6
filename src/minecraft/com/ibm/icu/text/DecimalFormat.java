  ...
  extends NumberFormat
{
  ...
  private StringBuffer format(double number, StringBuffer result, FieldPosition fieldPosition, boolean parseAttr)
  {
    ...
    if (Double.isNaN(number)) {
      ...
      if (parseAttr) {
        addAttribute(NumberFormat.Field.INTEGER, result.length() - symbols.getNaN().length(), result.length());
      }
    ...
    if (Double.isInfinite(number)) {
      ...
      if (parseAttr) {
        addAttribute(NumberFormat.Field.INTEGER, result.length() - symbols.getInfinity().length(), result.length());
      }
  ...
  {
    if (multiplier != 1) {
      number = number.multiply(com.ibm.icu.math.BigDecimal.valueOf(multiplier), mathContext);
    }
  ...
  private StringBuffer subformat(String pluralCount, StringBuffer result, FieldPosition fieldPosition, boolean isNegative, boolean isInteger, boolean parseAttr)
  {
    if (style == 6)
    {
      ...
      if (!formatPattern.equals(currencyPluralPattern)) {
  ...
  private void subformatFixed(StringBuffer result, FieldPosition fieldPosition, boolean isInteger, boolean parseAttr)
  {
    ...
    boolean fractionPresent = ((!isInteger) && (digitIndex < digitList.count)) || (useSigDig ? sigCount < minSigDig : getMinimumFractionDigits() > 0);
  ...
  private void subformatExponential(StringBuffer result, FieldPosition fieldPosition, boolean parseAttr)
  {
    ...
    if (parseAttr) {
      addAttribute(NumberFormat.Field.EXPONENT_SYMBOL, result.length() - symbols.getExponentSeparator().length(), result.length());
    }
  ...
  private Object parse(String text, ParsePosition parsePosition, Currency[] currency)
  {
    ...
    else if (!subparse(text, parsePosition, digitList, status, currency, negPrefixPattern, negSuffixPattern, posPrefixPattern, posSuffixPattern, 0))
    {
    ...
    if (status[0] != 0) {
      n = new Double(status[1] != 0 ? Double.POSITIVE_INFINITY : Double.NEGATIVE_INFINITY);



    }
    else if (status[2] != 0) {
      n = status[1] != 0 ? new Double("0.0") : new Double("-0.0");


    }
    else if ((status[1] == 0) && (digitList.isZero())) {
      n = new Double("-0.0");
        ...
        if (digitList.decimalAt < 12) {
          ...
          if (digitList.count > 0) {
            ...
            while (nx < digitList.count) {
              l = l * 10L + (char)digitList.digits[(nx++)] - 48L;
            }
          ...
          BigInteger big = digitList.getBigInteger(status[1]);
          n = big.bitLength() < 64 ? Long.valueOf(big.longValue()) : big;
        }
      }
      else
      {
        com.ibm.icu.math.BigDecimal big = digitList.getBigDecimalICU(status[1]);
        n = big;
        if (mult != 1) {
          n = big.divide(com.ibm.icu.math.BigDecimal.valueOf(mult), mathContext);
        }
      }
    }
  ...
  private boolean parseForCurrency(String text, ParsePosition parsePosition, Currency[] currency, boolean[] status)
  {
    ...
    boolean found;
    boolean found; if (style == 6) {
      found = subparse(text, tmpPos, tmpDigitList, tmpStatus, currency, negPrefixPattern, negSuffixPattern, posPrefixPattern, posSuffixPattern, 1);
    }
    else
    {
      found = subparse(text, tmpPos, tmpDigitList, tmpStatus, currency, negPrefixPattern, negSuffixPattern, posPrefixPattern, posSuffixPattern, 0);
    }
    ...
    for (AffixForCurrency affix : affixPatternsForCurrency) {
      ...
      boolean result = subparse(text, tmpPos, tmpDigitList, tmpStatus, currency, affix.getNegPrefix(), affix.getNegSuffix(), affix.getPosPrefix(), affix.getPosSuffix(), affix.getPatternType());
    ...
    currencySignCount = -1;
    boolean result = subparse(text, tmpPos, tmpDigitList, tmpStatus, currency, negativePrefix, negativeSuffix, positivePrefix, positiveSuffix, 0);
  ...
  private void setupCurrencyAffixForAllPatterns()
  {
    ...
    applyPatternWithoutExpandAffix(getPattern(symbols.getULocale(), 1), false);
    
    AffixForCurrency affixes = new AffixForCurrency(negPrefixPattern, negSuffixPattern, posPrefixPattern, posSuffixPattern, 0);
    ...
    while (iter.hasNext()) {
      ...
      if ((currencyPattern != null) && (!currencyUnitPatternSet.contains(currencyPattern)))
      {
        ...
        affixes = new AffixForCurrency(negPrefixPattern, negSuffixPattern, posPrefixPattern, posSuffixPattern, 1);
  ...
  private final boolean subparse(String text, ParsePosition parsePosition, DigitList digits, boolean[] status, Currency[] currency, String negPrefix, String negSuffix, String posPrefix, String posSuffix, int type)
  {
    ...
    else
    {
      ...
      UnicodeSet groupEquiv = strictParse ? strictDefaultGroupingSeparators : skipExtendedSeparatorParsing ? UnicodeSet.EMPTY : defaultGroupingSeparators;
      ...
      for (; position < text.length(); position += UTF16.getCharCount(ch)) {
        ...
        } else if ((digit > 0) && (digit <= 9))
        {
          ...
          if ((strictParse) && (
            (backup != -1) || ((lastGroup != -1) && (countCodePoints(text, lastGroup, position) != groupingSize + 1))))
          {
            strictFail = true;
            break;
          }
  ...
  private int compareComplexAffix(String affixPat, String text, int pos, int type, Currency[] currency)
  {
    ...
    for (int i = 0; (i < affixPat.length()) && (pos >= 0);) {
      ...
      if (c == '\'') {
        for (;;) {
          ...
          if (j > i) {
            ...
            i = j + 1;
            if ((i >= affixPat.length()) || (affixPat.charAt(i) != '\'')) break;
            pos = match(text, pos, 39);
            i++;
      ...
      case '¤': 
        ...
        boolean plural = (i < affixPat.length()) && (affixPat.charAt(i) == '¤');
        if (plural) {
          i++;
          intl = false;
        }
  ...
  public void setRoundingIncrement(java.math.BigDecimal newValue)
  {
    if (newValue == null) {
      ...
      setRoundingIncrement(new com.ibm.icu.math.BigDecimal(newValue));
    }
  }
  ...
  private void setRoundingDoubleReciprocal(double rawRoundedReciprocal) {
    ...
    if (Math.abs(rawRoundedReciprocal - roundingDoubleReciprocal) > 1.0E-9D) {
      roundingDoubleReciprocal = 0.0D;
    }
  }
  ...
  public java.math.MathContext getMathContext()
  {
    try
    {
      return mathContext == null ? null : new java.math.MathContext(mathContext.getDigits(), RoundingMode.valueOf(mathContext.getRoundingMode()));
    }
    catch (Exception foo) {}
    return null;
  }
  ...
  {
    mathContext = new com.ibm.icu.math.MathContext(newValue.getPrecision(), 1, false, newValue.getRoundingMode().ordinal());
  }
  ...
  public boolean equals(Object obj)
  {
    ...
    return (currencySignCount == currencySignCount) && ((style != 6) || ((equals(posPrefixPattern, posPrefixPattern)) && (equals(posSuffixPattern, posSuffixPattern)) && (equals(negPrefixPattern, negPrefixPattern)) && (equals(negSuffixPattern, negSuffixPattern)))) && (multiplier == multiplier) && (groupingSize == groupingSize) && (groupingSize2 == groupingSize2) && (decimalSeparatorAlwaysShown == decimalSeparatorAlwaysShown) && (useExponentialNotation == useExponentialNotation) && ((!useExponentialNotation) || (minExponentDigits == minExponentDigits)) && (useSignificantDigits == useSignificantDigits) && ((!useSignificantDigits) || ((minSignificantDigits == minSignificantDigits) && (maxSignificantDigits == maxSignificantDigits))) && (symbols.equals(symbols)) && (Utility.objectEquals(currencyPluralInfo, currencyPluralInfo));
  }
      ...
      case '¤': 
        ...
        if (currency != null)
        {
          ...
          if ((plural) && (pluralCount != null)) {
            ...
            s = currency.getName(symbols.getULocale(), 2, pluralCount, isChoiceFormat);
          }
          else if (!intl) {
            boolean[] isChoiceFormat = new boolean[1];
            s = currency.getName(symbols.getULocale(), 0, isChoiceFormat);
  ...
  private String toPattern(boolean localized)
  {
    ...
    if (roundingIncrementICU != null) {
      int i = roundingIncrementICU.scale();
    ...
    for (int part = 0; part < 2; part++)
    {
      ...
      int maxDig = 0;int minDig = 0;int maxSigDig = 0;
      if (useSigDig) {
      ...
      if (part == 0) {
        if ((negativeSuffix.equals(positiveSuffix)) && (negativePrefix.equals('-' + positivePrefix))) {
          break;
        }
  ...
  private void applyPatternWithoutExpandAffix(String pattern, boolean localized) {
    ...
    for (int part = 0; (part < 2) && (pos < pattern.length()); part++)
    {
      ...
      int decimalPos = -1;
      int multpl = 1;
      int digitLeftCount = 0;int zeroDigitCount = 0;int digitRightCount = 0;int sigDigitCount = 0;
      ...
      if (part == 0)
      {
        ...
        if (useSigDig) {
          ...
          setMaximumIntegerDigits(useExponentialNotation ? digitLeftCount + minInt : 309);
        ...
        if (incrementVal != 0L)
        {
          ...
          roundingIncrementICU = com.ibm.icu.math.BigDecimal.valueOf(incrementVal, scale > 0 ? scale : 0);
          if (scale < 0) {
    ...
    if (currencySignCount > 0)
    {
      ...
      if ((currencySignCount == 3) && (currencyPluralInfo == null))
      {
        currencyPluralInfo = new CurrencyPluralInfo(symbols.getULocale());
      }
    }
  ...
  public void setCurrency(Currency theCurrency)
  {
    ...
    if (theCurrency != null) {
      boolean[] isChoiceFormat = new boolean[1];
      String s = theCurrency.getName(symbols.getULocale(), 0, isChoiceFormat);
  ...
  static class Unit
  {
    ...
    public boolean equals(Object obj)
    {
      ...
      Unit other = (Unit)obj;
      return (prefix.equals(prefix)) && (suffix.equals(suffix));
    }
  }
