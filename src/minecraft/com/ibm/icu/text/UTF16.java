...
public final class UTF16
{
  ...
  {
    int size = source.length();int result = 0;int count = offset32;
  ...
  {
    int result = start;int count = offset32;
  ...
  public static int moveCodePointOffset(String source, int offset16, int shift32)
  {
    ...
    for (int count = -shift32; count > 0; count--) {
      result--;
      if (result < 0) {
        break;
      }
  ...
  public static String newString(int[] codePoints, int offset, int count)
  {
    ...
    int w = 0;
    int r = offset; for (int e = offset + count; r < e; r++) {
      int cp = codePoints[r];
      if ((cp < 0) || (cp > 1114111)) {
        throw new IllegalArgumentException();
      }
    ...
    implements Comparator<String>
  {
    ...
    {
      return Normalizer.cmpEquivFold(s1, s2, m_foldCase_ | m_codePointCompare_ | 0x10000);
    }
