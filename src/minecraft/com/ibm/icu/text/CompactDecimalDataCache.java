...
class CompactDecimalDataCache
{
  ...
  {
    DataBundle result = (DataBundle)cache.get(locale);
    if (result == null) {
      result = load(locale);
  ...
  private static DataBundle load(ULocale ulocale)
  {
    ...
    Data longData;
    Data longData; if (longDataBundle == null) {
      longData = shortData;
    } else {
  ...
  private static int populatePrefixSuffix(String pluralVariant, int idx, String template, ULocale locale, String style, Data result)
  {
    ...
    saveUnit(new DecimalFormat.Unit(prefix, suffix), pluralVariant, idx, units);
  ...
  private static String fixQuotes(String prefixOrSuffix) {
    ...
    for (int idx = 0; idx < len; idx++) {
      ...
      switch (1.$SwitchMap$com$ibm$icu$text$CompactDecimalDataCache$QuoteState[state.ordinal()]) {
      case 1: 
  ...
  private static void fillInMissing(Data result)
  {
    ...
    for (int i = 0; i < divisors.length; i++) {
      if (((DecimalFormat.Unit[])units.get("other"))[i] == null) {
        divisors[i] = lastDivisor;
        copyFromPreviousIndex(i, units);
      } else {
        lastDivisor = divisors[i];
        propagateOtherToMissing(i, units);
      }
    }
  }
