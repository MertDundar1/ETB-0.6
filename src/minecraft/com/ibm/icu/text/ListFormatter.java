...
public final class ListFormatter
{
  ...
  static void add(String locale, String... data) { localeToData.put(new ULocale(locale), new ListFormatter(data[0], data[1], data[2], data[3])); }
  
  private static class Cache
  {
    private final ICUCache<ULocale, ListFormatter> cache = new SimpleCache();
    
    private Cache() {}
    
    public ListFormatter get(ULocale locale) { ListFormatter result = (ListFormatter)cache.get(locale);
      if (result == null) {
        result = load(locale);
