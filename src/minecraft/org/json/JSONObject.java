...
public class JSONObject
{
  ...
  public JSONObject(Map<?, ?> m)
  {
    if (m == null) {
      ...
      for (Map.Entry<?, ?> e : m.entrySet()) {
        ...
        if (value != null) {
          map.put(String.valueOf(e.getKey()), wrap(value));
        }
      }
    }
  }
  ...
  private void populateMap(Object bean)
  {
    ...
    Method[] arrayOfMethod1; int j = (arrayOfMethod1 = methods).length; for (int i = 0; i < j;) { Method method = arrayOfMethod1[i];
  ...
  private String getKeyNameFromMethod(Method method) {
    ...
    String key; if ((name.startsWith("get")) && (name.length() > 3)) {
      key = name.substring(3); } else { String key;
